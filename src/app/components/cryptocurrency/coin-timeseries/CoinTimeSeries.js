...
const CoinTimeSeries = (props) => {
    const [active, setActive] = useState(cryptoIntervals[1]);
    ...
    const renderChart =
        chart === cryptoTimeSeriesCharts.candlestick ?
            <ReactApexChart
                ...
                type="candlestick"
                height={500}
            /> : <Line data={coinMarketChartData.chartData} options={timeSeriesCryptoOptions}
            ...
            <div className="card-body">
                <div className="d-flex">
                    <button className="btn btn-light border-right rounded-0 p-3"
                            onClick={() => setChart(chart === cryptoTimeSeriesCharts.candlestick ? cryptoTimeSeriesCharts.line : cryptoTimeSeriesCharts.candlestick)}>{renderCandlestickIcon}</button>
                    <button className="btn btn-light rounded-0 p-3"
                            onClick={() => setChart(chart === cryptoTimeSeriesCharts.candlestick ? cryptoTimeSeriesCharts.line : cryptoTimeSeriesCharts.candlestick)}>{renderLineIcon}</button>
                </div>
