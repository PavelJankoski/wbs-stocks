...
const StockTimeSeries = () => {
    ...
    useEffect(() => {
        dispatch(actions.fetchStocksForInterval("MSFT", active.interval, 10));
    }, [active, dispatch])
    ...
    const renderTimeSeries = stockIntervals.map((t) => (
        <li key={t.label} className="nav-item">
            <button className={`nav-link ${active.label === t.label ? 'active': ''}`} onClick={() => setActive(t)} data-toggle="tab" role="tab" aria-selected="false">{t.label}</button>
        </li>
    ))
        ...
        <div className="col-md-8 grid-margin stretch-card">
            <div className="card">
                {!stockInInterval.loading ? <div className="card-body">
                    <h4 className="mb-0 font-weight-medium">{stockInInterval.symbol}</h4>
                    <div className="d-xl-flex flex-column flex-lg-row">
                        <p>O:<span className="text-info">{formatStringToDecimal(stockInInterval.lastData?.open)}</span> H:<span className="text-info">{formatStringToDecimal(stockInInterval.lastData?.high)}</span> L:<span className="text-info">{formatStringToDecimal(stockInInterval.lastData?.low)}</span> C:<span className="text-info">{formatStringToDecimal(stockInInterval.lastData?.close)}</span></p>
                    ...
                    <Line data={stockInInterval.chartData} options={timeSeriesStockOptions}  datasetKeyProvider={datasetKeyProvider} height={50} width={100}/>
